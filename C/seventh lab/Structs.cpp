...
int AvaibleProgram(turProgram* Begin)
{
	...
	while (avaible) {
		if (avaible->EmptySpace > NULL) {
			...
			printf(" (Номер - %d, - свободных мест - %d), ", avaible->Number, avaible->EmptySpace);
			i++;
		}
...
{
	printf("Выберенте номер доступной турпрограммы, которую хотите заказать или exit для отмены заказа: ");
	int Passenger;
	int YourMoney = 0;
	int LoL = 0;
	for (;;) {
		...
		while (avaible) {
			if (Choice == avaible->Number) {
				...
				if (X) {
					...
					for (;;) {
						...
						if (Passenger > avaible->EmptySpace)
							printf("Количество свободных мест недостаточно для данного заказа. Введите меньшее число!\n");
						else
							break;
					}
				...
				printf("Выбрать полную программу или выбрать самостоятельно необязатьельные услуги?(Введите Y для выбора полной программмы, при нажатии другой клавиши - самостотельно): ");
				if (Obl()) {
				...
				Sleep(3000);
				system("cls");
				cout << "Выбрана турпрограмма: " << avaible->ProgramName << ", стоимость тура без учёта услуг: " << avaible->TourPrice << endl;
				printf("Заказано мест: %d, стоимость одного места: %d$\n", Passenger, YourMoney / Passenger);
		...
		if (!LoL)
			printf("Такой программы не существует. Выберете из списка доступных: ");
		else {
			*Money += YourMoney;
			break;
		}
